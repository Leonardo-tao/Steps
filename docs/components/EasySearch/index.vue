<template>
  <div class="easy-search">
    <el-form class="searchForm" :model="queryParams" ref="queryForm" inline label-width="auto">
      <slot></slot>
      <el-form-item v-for="(item, index) in exposedFields" :key="index" :label="item.label" :prop="item.prop">
          ...
          <el-table :data="conditions" :show-header="false" max-height="280px">
            <el-table-column prop="field" label="字段" width="150" align="center">
              <template v-slot="scope">
                <el-select v-model="scope.row.field" placeholder="字段" :clearable="false" @change="handleFieldChange(scope.row)">
            ...
            <el-table-column prop="operator" label="比较" width="100" align="center">
              <template v-slot="scope">
                <el-select v-model="scope.row.operator" placeholder="比较" :clearable="false" :disabled="!scope.row.modifiable" @change="handleOpChange(scope.row)">
                  <el-option label="等于" value="eq" />
                  <el-option label="不等于" value="ne" />
                  <el-option label="包含" value="ct" />
                  <el-option label="不包含" value="nc" />
                  <el-option label="为空" value="nu" />
                  <el-option label="不为空" value="nn" />
                  <el-option label="在...之间" value="bt" />
                </el-select>
              </template>
            </el-table-column>
...
<script>
import { listFieldConfig } from "@/api/basic/fieldConfig";
...
export default {
  ...
  data() {
    return {
      ...
      conditions: [],       // 条件列表
      exposeNum: 3,         // 默认显示字段数
      exposedFields: [],    // 初始化字段列表
    };
  },
  methods: {
    // 初始化字段列表
    async initFields() {
      const res = await listFieldConfig({ menuComponent: this.$route.path }).catch(() => {this.$message.error("获取字段列表失败，请检查该界面是否配置字段列表！")})
      this.fields = res.rows.filter(field => field.visible).sort((a, b) => a.orderNum - b.orderNum);
      ...
      // 初始化暴露的字段
      if (this.exposingFields?.length != 0) {
        // 筛选符合 exposingFields 的字段
        this.exposedFields = this.fields.filter(field =>
          this.exposingFields.includes(field.prop)
        );
      } else {
        // 默认显示前 exposeNum 个字段
        this.exposedFields = this.fields.slice(0, this.exposeNum);
      }
      ...
      console.log("exposedFields", this.exposedFields);
    },
    // 获取字段类型
    getFieldStyle(f) {
      const prop = f.toString().trim().toLowerCase();
      console.log(prop);
      if (prop.includes("time")) {
        return "datetime";
      }
      if (prop.includes("num")) {
        return "number";
      }
      return "string";
    },
    ...
    addCondition() {
      try {
        const field = this.fields[this.addIndex]?.prop ?? "";
    ...
    generateParams() {
      ...
      const result = this.exposedFields
        ...
        .filter(item => item.value?.toString().trim() || item.operator == "nn" || item.operator == "nu");
...
@import "@/components/EasySearch/index.scss";
</style>
