...
::: tip
试题来源：[前端面试派](https://www.mianshipai.com/docs/written-exam/JS-writing.html)
:::
...
---
第七行的 `hash.has(obj)` 的作用？

避免循环引用（ `a.b = a` ）造成的栈溢出问题，当检测到已经解析过该对象时，直接返回解析结果（内存地址）。这样可以 **保留原有的引用关系**：
...
---
为什么要加 `Object.prototype.hasOwnProperty.call(obj, key)` ？

去除掉原型链上继承来的其他属性

```js
const proto = { fromProto: 123 };
...
// 输出：own, fromProto   ← 把原型上的属性 fromProto 也枚举出来了
```

---
不足：
- 不支持：`Date/RegExp/Map/Set/ArrayBuffer/TypedArray/Error` 等
- 丢失：`Symbol` 键、不可枚举属性、`getter/setter`、属性描述符、原型
- 类实例会被“扁平化”为普通对象

::: details `chatGPT` 的改进
# 进阶一：补齐常见内建类型

下面这版在你的思路上做增强，**仍保留 WeakMap 防环**，并按类型标签精细处理。你可以“替换式学习”：一处处看并理解新的分支。
...
function cloneDeepPro(source, hash = new WeakMap()) {
  // 1) 原始类型 / 函数：直接返回
  ...
  if (tag === '[object Map]') {
    ...
    hash.set(source, res);
    source.forEach((v, k) => {
      res.set(cloneDeepPro(k, hash), cloneDeepPro(v, hash));
    });
    return res;
  }
  if (tag === '[object Set]') {
    const res = new Set();
    hash.set(source, res);
    source.forEach(v => res.add(cloneDeepPro(v, hash)));
    return res;
  }
  if (tag === '[object ArrayBuffer]') {
    return source.slice(0); // 拷贝底层内存
  }
  // TypedArray / DataView
  if (ArrayBuffer.isView(source)) {
    ...
    // 如 Uint8Array/Float32Array/BigInt64Array...
    return new source.constructor(source); // 值复制
  }
  if (tag === '[object Error]') {
    const e = new source.constructor(source.message);
    e.name = source.name;
    e.stack = source.stack;
    return e;
  }
  // 不可克隆或没意义深拷贝的类型（按引用返回）
  ...
  const target = Array.isArray(source) ? [] : Object.create(proto);
  hash.set(source, target);

  // 4) 复制“所有自有键”（含 Symbol 与不可枚举）
  ...
  for (const key of keys) {
    ...
    if ('value' in desc) {
      // 数据属性：递归克隆
      desc.value = cloneDeepPro(desc.value, hash);
    }
    // 访问器属性（get/set）保持原样引用
...
> 小练习：
> 
> 1. 用 `Map/Set/Date/RegExp/Uint8Array/Symbol 键/不可枚举属性/类实例` 逐个测试 `cloneDeepPro`。
>     
> 2. 写个类 `class User { constructor(name){ this.name = name } }`，拷贝实例后检查 `Object.getPrototypeOf(copy) === User.prototype` 是否为 `true`。
>     
...
- **函数要不要拷贝？**  
    一般**不要**克隆函数源码（除非你有极特殊需求），按引用足够且安全。
    
- **访问器属性（getter/setter）怎么处理？**  
    上面保留了原访问器本体（`get/set` 函数）——这是最接近语义的做法。若你想把 `getter` 的结果变成静态值，需要另写分支（但会改变对象语义）。
    
- **类实例与私有状态**  
    原型被保留，但**私有字段（`#x`）**与内部隐藏状态无法通过用户态代码复制——这是 JS 语言层面的限制。
    
- **DOM 节点**  
    如需克隆 DOM，用 `node.cloneNode(true)` 更合适，而不是通用深拷贝。
...
const a2 = cloneDeepPro(a);
console.log(a2 !== a, a2.self === a2);

// 2) Date / RegExp
const b = { d: new Date(), r: /hi/gi };
const b2 = cloneDeepPro(b);
console.log(b2.d instanceof Date, b2.d.getTime() === b.d.getTime());
console.log(b2.r instanceof RegExp, b2.r.source === b.r.source, b2.r.flags === b.r.flags);

// 3) Map / Set
const m = new Map([[{k:1}, new Set([1,2,3])]]);
const m2 = cloneDeepPro(m);
console.log(m2 !== m, [...m2.keys()][0] !== [...m.keys()][0]);

// 4) TypedArray / ArrayBuffer
const buf = new ArrayBuffer(4);
new Uint8Array(buf)[0] = 7;
const c = { buf, ta: new Uint8Array([1,2,3]) };
const c2 = cloneDeepPro(c);
console.log(c2.buf !== buf, new Uint8Array(c2.buf)[0] === 7);
console.log(c2.ta !== c.ta, c2.ta[0] === 1);

// 5) Symbol 键 / 不可枚举
const s = Symbol('s');
const d = {};
Object.defineProperty(d, 'hidden', { value: 10, enumerable: false });
d[s] = 99;
const d2 = cloneDeepPro(d);
console.log(Object.getOwnPropertyDescriptor(d2, 'hidden').enumerable === false);
console.log(d2[s] === 99);

// 6) 原型
class User { constructor(name){ this.name = name } }
const u = new User('Ada'); u.meta = { lang: 'JS' };
const u2 = cloneDeepPro(u);
console.log(Object.getPrototypeOf(u2) === User.prototype);
console.log(u2 !== u, u2.meta !== u.meta);
```

---

# 额外提示：内置的 `structuredClone`

在现代运行时（Node 17+/浏览器新版本）可以直接用：

`const copy = structuredClone(value);`

- 优点：快、覆盖面广、支持环、支持 `Map/Set/ArrayBuffer/TypedArray` 等。
    
- 局限：不拷贝函数、不可克隆 DOM 节点、会丢失原型（变成普通对象）。如果你需要“保留原型/描述符”，还是要用上面的 `cloneDeepPro`。

链接：[深拷贝实现解析](https://chatgpt.com/c/68b94406-9fdc-8326-ae70-44b848df617f)
:::
